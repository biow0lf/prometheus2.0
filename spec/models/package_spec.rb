...
describe Package do
  ...
  it "should import package to database" do
    ...
    Package.should_receive(:`).with("rpm -qp --queryformat='%{SOURCERPM}' #{file}").and_return('openbox-3.4.11.1-alt1.1.1.src.rpm')
    Package.should_receive(:`).with("rpm -qp --queryformat='%{NAME}' #{file}").and_return('openbox')
    Package.should_receive(:`).with("rpm -qp --queryformat='%{VERSION}' #{file}").and_return('3.4.11.1')
    Package.should_receive(:`).with("rpm -qp --queryformat='%{RELEASE}' #{file}").and_return('alt1.1.1')
    Package.should_receive(:`).with("rpm -qp --queryformat='%{EPOCH}' #{file}").and_return('(none)')
    Package.should_receive(:`).with("rpm -qp --queryformat='%{ARCH}' #{file}").and_return('i586')
    Package.should_receive(:`).with("rpm -qp --queryformat='%{GROUP}' #{file}").and_return('Graphical desktop/Other')
    Package.should_receive(:`).with("rpm -qp --queryformat='%{SUMMARY}' #{file}").and_return('short description')
    Package.should_receive(:`).with("rpm -qp --queryformat='%{LICENSE}' #{file}").and_return('GPLv2+')
    Package.should_receive(:`).with("rpm -qp --queryformat='%{URL}' #{file}").and_return('http://openbox.org/')
    Package.should_receive(:`).with("rpm -qp --queryformat='%{DESCRIPTION}' #{file}").and_return('long description')
    Package.should_receive(:`).with("rpm -qp --queryformat='%{BUILDTIME}' #{file}").and_return('1315301838')
