...
#  match '(/:locale)(/:branch)/packager/:login/repocop/rss' => 'maintainer#repocop', :constraints => { :locale => SUPPORTED_LOCALES, :branch => /[^\/]+/ }
  ...
  match '(/:locale)/srpm(/:branch)/:name' => 'srpm#main', :as => 'srpm_main', :constraints => { :locale => SUPPORTED_LOCALES, :branch => /[^\/]+/, :name => SRPMS_NAME }
  match '(/:locale)/srpm(/:branch)/:name/changelog' => 'srpm#changelog', :as => 'srpm_changelog', :constraints => { :locale => SUPPORTED_LOCALES, :branch => /[^\/]+/, :name => SRPMS_NAME }
