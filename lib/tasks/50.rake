...
namespace :"50" do
  ...
  task :update => :environment do
    ...
    puts "#{Time.now.to_s}: update *.i586.rpm/*.noarch.rpm/*.x86_64.rpm from 5.0 to database"
  ...
  task :srpms => :environment do
    require 'open-uri'
    puts "#{Time.now.to_s}: import *.src.rpm from 5.0 to database"
    branch = Branch.where(name: '5.0', vendor: 'ALT Linux').first
    Srpm.import_all(branch, '/ALT/5.0/files/SRPMS/*.src.rpm')
    puts "#{Time.now.to_s}: end"
  end

  desc 'Import *.i586.rpm/*.noarch.rpm/*.x86_64.rpm from 5.0 to database'
  task :binary => :environment do
    require 'open-uri'
    puts "#{Time.now.to_s}: import *.i586.rpm/*.noarch.rpm/*.x86_64.rpm from 5.0 to database"
    branch = Branch.where(name: '5.0', vendor: 'ALT Linux').first
    pathes = ['/ALT/5.0/files/i586/RPMS/*.i586.rpm',
              '/ALT/5.0/files/noarch/RPMS/*.noarch.rpm',
              '/ALT/5.0/files/x86_64/RPMS/*.x86_64.rpm']
