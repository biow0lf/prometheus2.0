...
task :acls => :environment do
  ...
  Acl.update_acls 'ALT Linux', '5.0', 'http://git.altlinux.org/acl/list.packages.5.0'
...
task :groups => :environment do
  ...
  Group.update_groups 'ALT Linux', '5.0', 'http://git.altlinux.org/gears/r/rpm.git?p=rpm.git;a=blob_plain;f=GROUPS'
  puts Time.now.to_s + ": end"
end
...
task :srpms => :environment do
  ...
  Srpm.import_srpms 'ALT Linux', '5.0'
  puts Time.now.to_s + ": end"
end
...
task :i586 => :environment do
  ...
  Package.import_packages_i586 'ALT Linux', '5.0'
  puts Time.now.to_s + ": end"
end
...
task :noarch => :environment do
  ...
  Package.import_packages_noarch 'ALT Linux', '5.0'
  puts Time.now.to_s + ": end"
end
...
task :x86_64 => :environment do
  ...
  Package.import_packages_x86_64 'ALT Linux', '5.0'
  puts Time.now.to_s + ": end"
end
...
task :leaders => :environment do
  ...
  Leader.update_leaders 'ALT Linux', '5.0', 'http://git.altlinux.org/acl/list.packages.5.0'
  puts Time.now.to_s + ": end"
end

desc "Import packagers list from src.rpm from 5.0 to database"
task :packagers => :environment do
  require 'rpm'
  puts Time.now.to_s + ": import src.rpm's"
  Packager.update_packager_list 'ALT Linux', '5.0'
  puts Time.now.to_s + ": end"
end
...
task :teams => :environment do
  ...
  Team.update_from_gitalt 'ALT Linux', '5.0'
  puts Time.now.to_s + ": end"
end
