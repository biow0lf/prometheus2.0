namespace :p5 do
  ...
  task :update => :environment do
    require 'open-uri'
    Rails.logger.info("#{Time.now.to_s}: Update p5 stuff")
    ...
    Rails.logger.info("#{Time.now.to_s}: update *.src.rpm from p5 to database")
    ...
    Rails.logger.info("#{Time.now.to_s}: update *.i586.rpm/*.noarch.rpm/*.x86_64.rpm from Platform5 to database")
