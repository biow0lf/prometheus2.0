...
namespace :p6 do
  ...
  task update: :environment do
    puts "#{ Time.zone.now }: Update p6 stuff"
    ...
    puts "#{ Time.zone.now }: update *.src.rpm from p6 to database"
    branch = Branch.find_by!(name: 'Platform6')
    Srpm.import_all(branch, '/ALT/p6/files/SRPMS/*.src.rpm')
    RemoveOldSrpms.call(branch, %w(/ALT/p6/files/SRPMS/)) do
      on(:ok) { puts "#{ Time.zone.now }: Old srpms removed" }
    end
    puts "#{ Time.zone.now }: update *.i586.rpm/*.noarch.rpm/*.x86_64.rpm from p6 to database"
