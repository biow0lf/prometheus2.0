namespace :p6 do
  ...
  task :update => :environment do
    require 'open-uri'
    Rails.logger.info "#{ Time.now }: Update p6 stuff"
    ...
    Rails.logger.info "#{ Time.now }: update *.src.rpm from p6 to database"
    ...
    Rails.logger.info "#{ Time.now }: update *.i586.rpm/*.noarch.rpm/*.x86_64.rpm from p6 to database"
