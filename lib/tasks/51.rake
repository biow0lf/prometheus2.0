...
task :acls => :environment do
  ...
  Acl.update_acls 'ALT Linux', '5.1', 'http://git.altlinux.org/acl/list.packages.5.1'
...
task :groups => :environment do
  ...
  Group.update_groups 'ALT Linux', '5.1', 'http://git.altlinux.org/gears/r/rpm.git?p=rpm.git;a=blob_plain;f=GROUPS'
  puts Time.now.to_s + ": end"
end
...
task :leaders => :environment do
  ...
  Leader.update_from_gitalt 'ALT Linux', '5.1'
  puts Time.now.to_s + ": end"
end
...
task :packagers => :environment do
  require 'rpm'
  puts Time.now.to_s + ": import src.rpm's"
  Packager.update_packager_list 'ALT Linux', '5.1'
  puts Time.now.to_s + ": end"
end
...
task :teams => :environment do
  ...
  Team.update_from_gitalt 'ALT Linux', '5.1'
  puts Time.now.to_s + ": end"
end
