namespace :"51" do
  ...
  task :update => :environment do
    require 'open-uri'
    Rails.logger.info("#{Time.now.to_s}: Update 5.1 stuff")
    ...
    Rails.logger.info("#{Time.now.to_s}: update *.src.rpm from 5.1 to database")
    ...
    Rails.logger.info("#{Time.now.to_s}: update *.i586.rpm/*.noarch.rpm/*.x86_64.rpm from 5.1 to database")
