namespace :t7 do
  ...
  task :update => :environment do
    require 'open-uri'
    puts "#{Time.now.to_s}: Update t7 stuff"
    ...
    puts "#{Time.now.to_s}: update *.src.rpm from t7 to database"
    ...
    puts "#{Time.now.to_s}: update *.i586.rpm/*.noarch.rpm/*.x86_64.rpm from t7 to database"
