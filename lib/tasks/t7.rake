namespace :t7 do
  ...
  task :update => :environment do
    ...
    Rails.logger.info("#{Time.now.to_s}: update *.src.rpm from t7 to database")
    ...
    Rails.logger.info("#{Time.now.to_s}: update *.i586.rpm/*.noarch.rpm/*.x86_64.rpm from t7 to database")
