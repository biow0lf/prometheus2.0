namespace :"40" do
  ...
  task :update => :environment do
    require 'open-uri'
    Rails.logger.info("#{Time.now.to_s}: Update 4.0 stuff")
    ...
    Rails.logger.info("#{Time.now.to_s}: update *.src.rpm from 4.0 to database")
    ...
    Rails.logger.info("#{Time.now.to_s}: update *.i586.rpm/*.noarch.rpm/*.x86_64.rpm from 4.0 to database")
