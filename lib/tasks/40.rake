namespace :"40" do
  ...
  desc "Import maintainers list from src.rpm from 4.0 to database"
  task :maintainers => :environment do
    require 'rpm'
    puts Time.now.to_s + ": import src.rpm's"
    Maintainer.import_maintainers_list "/ALT/4.0/files/SRPMS/*.src.rpm"
    puts Time.now.to_s + ": end"
  end
  ...
  task :teams => :environment do
    ...
    Team.update_from_gitalt 'ALT Linux', '4.0', 'http://git.altlinux.org/acl/list.groups.4.0'
    puts Time.now.to_s + ": end"
  end
end