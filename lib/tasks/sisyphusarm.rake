namespace :sisyphusarm do
  ...
  task :update => :environment do
    require 'open-uri'
    Rails.logger.info("#{Time.now.to_s}: Update SisyphusARM stuff")
    ...
    Rails.logger.info("#{Time.now.to_s}: update *.src.rpm from SisyphusARM to database")
    ...
    Rails.logger.info("#{Time.now.to_s}: update *.arm.rpm/*.noarch.rpm from SisyphusARM to database")
  ...
  task :srpms => :environment do
    require 'open-uri'
    Rails.logger.info("#{Time.now.to_s}: import *.src.rpm from SisyphusARM to database")
    branch = Branch.where(name: 'SisyphusARM', vendor: 'ALT Linux').first
    Srpm.import_all(branch, '/ALT/Sisyphus/arm/SRPMS.all/*.src.rpm')
    Rails.logger.info("#{Time.now.to_s}: end")
  end
  ...
  task :binary => :environment do
    require 'open-uri'
    Rails.logger.info("#{Time.now.to_s}: import *.arm.rpm/*.noarch.rpm from SisyphusARM to database")
    branch = Branch.where(name: 'SisyphusARM', vendor: 'ALT Linux').first
    pathes = ['/ALT/Sisyphus/files/arm/RPMS/*.rpm']
    Package.import_all(branch, pathes)
    Rails.logger.info("#{Time.now.to_s}: end")
