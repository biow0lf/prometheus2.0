namespace :"41" do
  ...
  task :update => :environment do
    require 'open-uri'
    Rails.logger.info("#{Time.now.to_s}: Update 4.1 stuff")
    ...
    Rails.logger.info("#{Time.now.to_s}: update *.src.rpm from 4.1 to database")
    ...
    Rails.logger.info("#{Time.now.to_s}: update *.i586.rpm/*.noarch.rpm/*.x86_64.rpm from 4.1 to database")
