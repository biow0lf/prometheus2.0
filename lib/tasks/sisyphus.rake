namespace :sisyphus do
  ...
  task :update => :environment do
    require 'open-uri'
    Rails.logger.info("#{Time.now.to_s}: Update Sisyphus stuff")
    ...
    Rails.logger.info("#{Time.now.to_s}: update *.i586.rpm/*.noarch.rpm/*.x86_64.rpm from Sisyphus to database")
  ...
  #   require 'open-uri'
  #   Rails.logger.info("#{Time.now.to_s}: import all leaders for packages from Sisyphus to database")
  ...
  #   require 'open-uri'
  #   Rails.logger.info("#{Time.now.to_s}: import all teams from Sisyphus to database")
